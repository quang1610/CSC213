...
static int socket_connect(char* server_name, unsigned short port) {
  ...
  // Open a socket
  /**
   * This function creates a socket. It takes inputs:
   *    - int domain: specified the communication domain. AF_INET = IPv4 protocol, there are also other types such as
   *        AF_BLUETOOTH
   *    - int type: specifies communication semantic. SOCK_STREAM means 2 ways binary stream. There are also datagram etc ...
  ...
  // Set up an address, this structure handles internet address
  struct sockaddr_in addr = {
    .sin_family = AF_INET,      // This is an internet socket
    .sin_port = htons(port)     // Connect to the appropriate port number. htons convert the unsigned short from host
                                // byte order to network byte order.
  };
  
  // Copy the server address info returned by gethostbyname into the address
  // Basically save the server information into
  memcpy(&addr.sin_addr.s_addr, server->h_addr, server->h_length);
  
  // Connect to the server
  /**
   * All that was to prepare for this function
   */
...
static int server_socket_open(unsigned short* port) {
  ...
  // here the OS will get us a new socket in .sin_port
