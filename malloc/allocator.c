...
__attribute__((constructor)) void init_heap() {
  int current_size = MIN_MALLOC_SIZE / 2;
  ...
  for (int i = 0; i < 8; i ++) {
    ...
    free_linked_lists[i] = (node_t*) ((char*)new_chunk + current_size);
    node_t* cursor = free_linked_lists[i];
    long n_loops = (long) (PAGE_SIZE/current_size);
    printf ("Debug, Allocating chunk for %d, looping %lu time\n", current_size, n_loops);
    
    for (long j = 2; j < n_loops; j ++) {
      cursor->next = (node_t*) ((char*)cursor + current_size);
      cursor = cursor->next;
    }
    cursor->next = NULL;
  }
...
void* xxmalloc(size_t size) {
  if (size > PAGE_SIZE/2) {
    ...
    if (p != NULL) {
      free_linked_lists[free_linked_lists_index] = p -> next;
      return p;
    } else {
      /// realize that we run out of space, allocate using mmap
      size = ROUND_UP(size, PAGE_SIZE);
      void* new_chunk =  mmap(NULL, size, PROT_READ|PROT_WRITE, MAP_ANONYMOUS|MAP_PRIVATE, -1, 0);
      ...
      /// allocate the free linked list
      /// set the array elem point to first elem of free linked list
      free_linked_lists[free_linked_lists_index] = (node_t*) ((char*)new_chunk + size);
      node_t* cursor = free_linked_lists[free_linked_lists_index];
      long n_loops = (long) (PAGE_SIZE/size - 2);
      printf ("Debug, Allocating chunk for %zu, looping %lu time\n", size, n_loops);
    
      for (long j = 0; j < n_loops; j ++) {
      ...
      /// set return value
      p = free_linked_lists[free_linked_lists_index];
      free_linked_lists[free_linked_lists_index] = p->next;
